...
export const kpIndexToSeverity = (kp: number): GeomagneticSeverity => {
  ...
  if (roundedKp >= 9) return { text: "Extreme", scale: "G5" };
  if (roundedKp >= 8) return { text: "Severe", scale: "G4" };
  if (roundedKp >= 7) return { text: "Strong", scale: "G3" };
  if (roundedKp >= 6) return { text: "Moderate", scale: "G2" };
  if (roundedKp >= 5) return { text: "Minor", scale: "G1" };
